...
const getVideosWithNameTag = (req: Request, res: Response, next: NextFunction) => {
    ...
    return Tag.findById(tagId)
        .then(async (oldTag) => {
            if (oldTag === null) {
                ...
                await Video.updateMany({ _id: addedVideos }, { $addToSet: { tag: newTag?._id } });
                await Video.updateMany({ _id: removedVideos }, { $pull: { tag: newTag?._id } });
...
const deleteTag = (req: Request, res: Response, next: NextFunction) => {
    ...
    return Tag.findByIdAndDelete(tagId)
        .then((tag) => Video.updateMany({ _id: tag?.videos }, { $pull: { tag: tag?._id } }))
